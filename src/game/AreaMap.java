...
public class AreaMap implements Session.Saveable {
  ...
  Table <Area, AreaTile> transitPoints = new Table();
  ...
  public void saveState(Session s) throws Exception {
    ...
    for (Area c : transitPoints.keySet()) {
      s.saveObject(c);
  ...
  /**  Tiles and related setup/query methods-
    */
  static AreaTile loadTile(AreaMap map, Session s) throws Exception {
    int x = s.loadInt();
    if (x == -1) return null;
    int y = s.loadInt();
  ...
  static void saveTile(AreaTile t, AreaMap map, Session s) throws Exception {
    if (t != null && map == null) {
  ...
  public static AreaTile[] adjacent(AreaTile spot, AreaTile temp[], AreaMap map) {
