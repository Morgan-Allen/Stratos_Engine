...
public class TerrainPass {
  ...
  private Batch <TerrainChunk>
    ...
    overlays = new Batch()
  ;
  private Batch <FlatTerrain.Chunk>
    flatChunks = new Batch()
  ;
  ...
  protected void register(FlatTerrain.Chunk chunk) {
  ...
  public void performPass() {
    //if (chunks.empty() && flatChunks.empty()) return;
    ...
    if (flatChunks.size() > 0) {
      renderFlatChunks(flatChunks);
    }
  ...
  protected void renderFlatChunks(Batch <FlatTerrain.Chunk> chunks) {
    
    final Colour c = Colour.WHITE;
    final float time = Rendering.activeTime();
    float opacity = 1;
    
    for (FlatTerrain.Chunk chunk : chunks) {
      final FlatTerrain terrain = chunk.belongs;
      final int frame = (int) ((time * 8) % terrain.animFrames);
      
      terrain.texture.bind(0);
      shader.setUniformf("u_texColor", c.r, c.g, c.b, 1);
      shader.setUniformf("u_opacity", opacity * c.a);
      
      chunk.frameGeom[frame].renderWithShader(shader, false);
