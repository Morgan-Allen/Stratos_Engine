...
public class Text extends UIGroup implements Description {
  ...
  final public static Colour
    LINK_COLOUR   = new Colour(0.1f, 0.3f, 0.4f, 1),
    ACTIVE_COLOUR = new Colour(0.0f, 0.5f, 0.0f, 1),
    HOVER_COLOUR  = new Colour(1.0f, 1.0f, 0.0f, 1);
  ...
  public Colour defaultColour = Colour.BLACK;
  ...
  public static Button insert(
    ...
    final Button linked = new Button(UI, null, texGraphic, null, link.fullName());
  ...
  protected void whenClicked() {
    final Clickable link = getTextSelection(UI.mousePos(), scrolled);
    if (link != null) whenLinkClicked(link);
  }
  
  
  protected void whenLinkClicked(Clickable link) {
    link.whenLinkClicked(this);
  }
  ...
  protected boolean renderText(
    ...
    if (link != null && entry.link == link) {
      ...
      final Colour c = entry.colour != null ? entry.colour : defaultColour;
