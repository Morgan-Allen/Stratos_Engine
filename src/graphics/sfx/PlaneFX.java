...
public class PlaneFX extends SFX {
  ...
  public static class Model extends graphics.common.ModelAsset {
    ...
    public boolean hasAnimation(String name) { return false; }
  ...
  protected void renderInPass(SFXPass pass) {
    ...
    final float rx = radius * scale * scaleX;
    final float ry = radius * scale * scaleY;
    ...
    Box2D frameUV;
    if (model.duration > 0) {
      final int maxFrame = model.animUV.length;
      final int frame = Nums.clamp((int) (progress * maxFrame), maxFrame);
    ...
    int i = 0; for (Vec3D v : verts) {
      ...
      v.x = (v.x - 0.5f) * rx * 2 * model.bounds.xdim();
      v.y = (v.y - 0.5f) * ry * 2 * model.bounds.ydim();
      v.z = 0;
      trans.trans(v);
