...
public class CutoutsPass {
  ...
  final Batch <CutoutSprite>
    ...
    passPlacing = new Batch <CutoutSprite> ();
  ...
  protected void register(CutoutSprite sprite) {
    switch (sprite.passType) {
      ...
      case (CutoutSprite.PASS_PLACING): passPlacing.add(sprite); break;
    }
  }
  ...
  private void performPass(Batch <CutoutSprite> inPass) {
    ...
    for (List <CutoutSprite> subPass : passLists) {
      subPass.queueSort();
      if (report) {
        Object model = subPass.first().model.sortingKey();
        I.say("  Rendering pass for "+model);
